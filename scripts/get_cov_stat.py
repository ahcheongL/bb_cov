...
def main(argv):
    if len(argv) < 2:
        print(f"Usage: {argv[0]} <cov_output_dir> [<output.json>]")
        print(
    ...
    cov_dir = argv[1]
    if not os.path.exists(cov_dir) or not os.path.isdir(cov_dir):
        print(f"Cov dir {cov_dir} does not exist or is not a directory")
        return 1
